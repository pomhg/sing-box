...
func TestURLTestWeightedRoundRobinSharesSocketAndFlowSequence(t *testing.T) {
	outbounds := []adapter.Outbound{newTestURLTestOutbound("a"), newTestURLTestOutbound("b")}
	history := newURLTestHistory(map[string]uint16{"a": 100, "b": 200})
	balancer, err := newURLTestBalancer(URLTestBalanceModeWeightedRoundRobin, outboundTags(outbounds), []option.URLTestWeight{{Outbound: "a", Weight: 3}})
	require.NoError(t, err)
	var tags []string
	for index := range 8 {
		// NFQUEUE can preview the packet before the actual socket or L3 flow.
		balancer.selectFlowOutbound(N.NetworkUDP, M.Socksaddr{}, outbounds, history, 0)
		var selected adapter.Outbound
		if index%2 == 0 {
			var commit func() func()
			selected, commit = balancer.selectFlowOutbound(N.NetworkUDP, M.Socksaddr{}, outbounds, history, 0)
			require.Nil(t, commit())
		} else {
			selected, _, _ = balancer.selectOutbound(N.NetworkUDP, M.Socksaddr{}, outbounds, history, 0, true)
		}
		tags = append(tags, selected.Tag())
	}
	require.Equal(t, []string{"a", "a", "b", "a", "a", "a", "b", "a"}, tags)
}
