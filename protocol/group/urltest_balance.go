...
func (b *urlTestBalancer) selectOutbound(network string, destination M.Socksaddr, outbounds []adapter.Outbound, history urlTestHistoryReader, tolerance uint16, acquire bool) (adapter.Outbound, bool, func()) {
	b.access.Lock()
	defer b.access.Unlock()
	selected, available, candidates := b.chooseOutboundLocked(network, destination, outbounds, history, tolerance, acquire)
	if selected == nil {
	...
	return selected, available, b.commitSelectionLocked(network, candidates, selected)
}
...
func (b *urlTestBalancer) selectFlowOutbound(network string, destination M.Socksaddr, outbounds []adapter.Outbound, history urlTestHistoryReader, tolerance uint16) (adapter.Outbound, func() func()) {
	b.access.Lock()
	defer b.access.Unlock()
	selected, _, candidates := b.chooseOutboundLocked(network, destination, outbounds, history, tolerance, true)
	if selected == nil {
		return nil, nil
	}
	return selected, func() func() {
		b.access.Lock()
		defer b.access.Unlock()
		return b.commitSelectionLocked(network, candidates, selected)
	}
}

// Commit only for an actual dial or a successfully created L3 flow.
func (b *urlTestBalancer) commitSelectionLocked(network string, candidates []adapter.Outbound, selected adapter.Outbound) func() {
...
func (b *urlTestBalancer) chooseOutboundLocked(network string, destination M.Socksaddr, outbounds []adapter.Outbound, history urlTestHistoryReader, tolerance uint16, advance bool) (adapter.Outbound, bool, []adapter.Outbound) {
	...
	if b.mode == URLTestBalanceModeFailover {
		return candidates[0], available, candidates
	}
	if !advance {
		if selected := b.selected[network]; slices.Contains(candidates, selected) {
			return selected, available, candidates
		}
		return candidates[0], available, candidates
	}
	switch b.mode {
	case URLTestBalanceModeRoundRobin:
		...
		return candidates[index%uint64(len(candidates))], available, candidates
	case URLTestBalanceModeRandom:
		return candidates[rand.IntN(len(candidates))], available, candidates
	case URLTestBalanceModeLeastConnection:
		...
		index := b.roundRobinIndex[network]
		return leastLoaded[index%uint64(len(leastLoaded))], available, candidates
	case URLTestBalanceModeWeightedRoundRobin:
		return b.chooseWeightedRoundRobinLocked(network, candidates), available, candidates
	case URLTestBalanceModeConsistentHash:
		return chooseConsistentHash(network, destination, candidates), available, candidates
	default:
...
func (b *urlTestBalancer) chooseWeightedRoundRobinLocked(network string, candidates []adapter.Outbound) adapter.Outbound {
...
func (b *urlTestBalancer) refresh(network string, outbounds []adapter.Outbound, history urlTestHistoryReader, tolerance uint16) bool {
	...
	if b.mode == URLTestBalanceModeFailover {
		...
		return pending || current != nil && selected != current
	}
	tags := make([]string, 0, len(candidates))
	for _, candidate := range candidates {
		tags = append(tags, candidate.Tag())
	}
	previous, initialized := b.available[network]
	changed := initialized && !slices.Equal(previous, tags)
	b.available[network] = tags
	if current := b.selected[network]; !slices.Contains(candidates, current) {
		if len(candidates) > 0 {
